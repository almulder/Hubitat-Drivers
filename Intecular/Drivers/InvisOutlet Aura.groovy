 ...
 * physical InvisOutlet Aura unit, created and managed by the "InvisOutlet Device
 * Service" app - you select which discovered units to add during that app's setup
 * wizard; you should not need to create this device manually. Adds color and
 * color-temperature control for its RGB "Color Light", which the Pro's plain white
 * nightlight doesn't have - the app picks this driver automatically for units it
 * detects as an Aura.
 *
 * OUTLETS / LIGHT - controlled explicitly by name, no ambiguous on()/off():
 *   - outlet1On() / outlet1Off()
 *   - outlet2On() / outlet2Off()
 *   - nightlightOn() / nightlightOff()
 *   - setLevel(0-100) dims the light.
 *   - setColor(), setHue(), setSaturation() - RGB color control.
 *   - setColorTemperature() - white color temperature in Kelvin.
 ...
 *
 * SENSORS: illuminance uses the standard capability attribute. Air quality,
 * motion, occupancy, distance, etc. are Pro-only and won't appear here since the
 * Aura's hardware doesn't have those sensors.
 *
...
def driverName() { "InvisOutlet Aura" }

metadata {
    definition(name: "InvisOutlet Aura", namespace: "almulder", author: "Albert Mulder") {
        ...
        attribute "colorMode", "enum", ["RGB", "CT"]
    }
...
def setLevel(level, duration = null) {
    if (logEnable) log.debug "${device.displayName}: requesting light level ${level}"
...
def setHue(hue) {
    def sat = (device.currentValue("saturation") ?: 100) as Double
    if (logEnable) log.debug "${device.displayName}: requesting hue ${hue}"
    parent?.sendHueSatCommand(device, "nightlight", hue, sat)
}

def setSaturation(saturation) {
    def hue = (device.currentValue("hue") ?: 0) as Double
    if (logEnable) log.debug "${device.displayName}: requesting saturation ${saturation}"
    parent?.sendHueSatCommand(device, "nightlight", hue, saturation)
}
...
def setColor(colorMap) {
    ...
    if (logEnable) log.debug "${device.displayName}: requesting color hue=${hue} sat=${sat}"
    parent?.sendHueSatCommand(device, "nightlight", hue, sat)
    if (colorMap?.level != null) setLevel(colorMap.level)
}

def setColorTemperature(colortemperature, level = null, transitionTime = null) {
    if (logEnable) log.debug "${device.displayName}: requesting color temperature ${colortemperature}K"
    parent?.sendColorTempCommand(device, "nightlight", colortemperature)
...
def updateColor(huePercent, satPercent) {
    if (logEnable) log.debug "${device.displayName}: updateColor(hue=${huePercent}, saturation=${satPercent})"
    sendEvent(name: "hue", value: huePercent)
    sendEvent(name: "saturation", value: satPercent)
    sendEvent(name: "colorMode", value: "RGB")
}

def updateColorTemperature(kelvin) {
    if (logEnable) log.debug "${device.displayName}: updateColorTemperature(${kelvin}K)"
